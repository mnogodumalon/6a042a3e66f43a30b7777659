...
const KUNDEN_FIELDS = [
  ...
  { key: 'strasse', label: 'Strasse', type: 'string/text' },
  ...
  { key: 'kunden_notizen', label: 'Notizen', type: 'string/textarea' },
];
const VERMIETUNG_FIELDS = [
  { key: 'werkzeug', label: 'Werkzeug', type: 'applookup/select', targetEntity: 'werkzeugbestand', targetAppId: 'WERKZEUGBESTAND', displayField: 'bezeichnung' },
  { key: 'kunde', label: 'Kunde', type: 'applookup/select', targetEntity: 'kunden', targetAppId: 'KUNDEN', displayField: 'nachname' },
  { key: 'mietbeginn', label: 'Mietbeginn', type: 'date/datetimeminute' },
  { key: 'mietende_geplant', label: 'Geplantes Mietende', type: 'date/datetimeminute' },
  { key: 'mietdauer_tage', label: 'Mietdauer (Tage)', type: 'number' },
  { key: 'mietpreis_gesamt', label: 'Mietpreis gesamt (EUR)', type: 'number' },
  { key: 'kaution_erhoben', label: 'Kaution erhoben (EUR)', type: 'number' },
  { key: 'kaution_zurueck', label: 'Kaution zurueckerstattet', type: 'bool' },
  { key: 'rueckgabedatum', label: 'Tatsaechliches Rueckgabedatum', type: 'date/datetimeminute' },
  { key: 'zustand_rueckgabe', label: 'Zustand bei Rueckgabe', type: 'lookup/select', options: [{ key: 'einwandfrei', label: 'Einwandfrei' }, { key: 'leichte_spuren', label: 'Leichte Gebrauchsspuren' }, { key: 'starke_spuren', label: 'Starke Gebrauchsspuren' }, { key: 'beschaedigt', label: 'Beschaedigt' }, { key: 'defekt', label: 'Defekt' }] },
...
export default function AdminPage() {
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'vermietung' && fieldKey === 'kunde') {
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'vermietung' || dialogState?.entity === 'vermietung') && (
        <VermietungDialog
          open={createEntity === 'vermietung' || dialogState?.entity === 'vermietung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'vermietung' ? handleUpdate : (fields: any) => handleCreate('vermietung', fields)}
          defaultValues={dialogState?.entity === 'vermietung' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'vermietung' && (
        <VermietungViewDialog
          open={viewState?.entity === 'vermietung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
