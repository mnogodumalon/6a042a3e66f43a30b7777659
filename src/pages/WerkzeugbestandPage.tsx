...
export default function WerkzeugbestandPage() {
  const [records, setRecords] = useState<Werkzeugbestand[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Werkzeugbestand | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Werkzeugbestand | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Werkzeugbestand | null>(null);
  ...
  async function handleCreate(fields: Werkzeugbestand['fields']) {
    await LivingAppsService.createWerkzeugbestandEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Werkzeugbestand['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateWerkzeugbestandEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteWerkzeugbestandEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Werkzeugbestand im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Werkzeugbestand suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anschaffungspreis')}>
                <span className="inline-flex items-center gap-1">
                  Anschaffungspreis (EUR)
                  {sortKey === 'anschaffungspreis' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('vermietbar')}>
                <span className="inline-flex items-center gap-1">
                  Zur Vermietung verfuegbar
                  {sortKey === 'vermietbar' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('tagesmietpreis')}>
                <span className="inline-flex items-center gap-1">
                  Tagesmietpreis (EUR)
                  {sortKey === 'tagesmietpreis' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('kaution_betrag')}>
                <span className="inline-flex items-center gap-1">
                  Kautionsbetrag (EUR)
                  {sortKey === 'kaution_betrag' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.anschaffungspreis ?? '—'}</TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.vermietbar ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.vermietbar ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell>{record.fields.tagesmietpreis ?? '—'}</TableCell>
                <TableCell>{record.fields.kaution_betrag ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={16} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Werkzeugbestand. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
