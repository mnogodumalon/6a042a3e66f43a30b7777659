...
interface VermietungDialogProps {
  open: boolean;
  onClose: () => void;
...
export function VermietungDialog({ open, onClose, onSubmit, defaultValues, recordId, werkzeugbestandList, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: VermietungDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="werkzeug" entity="Werkzeugbestand">\n${JSON.stringify(werkzeugbestandList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      ...
      const schema = `{\n  "werkzeug": string | null, // Display name from Werkzeugbestand (see <available-records>)\n  "kunde": string | null, // Display name from Kunden (see <available-records>)\n  "mietbeginn": string | null, // YYYY-MM-DDTHH:MM\n  "mietende_geplant": string | null, // YYYY-MM-DDTHH:MM\n  "mietdauer_tage": number | null, // Mietdauer (Tage)\n  "mietpreis_gesamt": number | null, // Mietpreis gesamt (EUR)\n  "kaution_erhoben": number | null, // Kaution erhoben (EUR)\n  "kaution_zurueck": boolean | null, // Kaution zurueckerstattet\n  "rueckgabedatum": string | null, // YYYY-MM-DDTHH:MM\n  "zustand_rueckgabe": LookupValue | null, // Zustand bei Rueckgabe (select one key: "einwandfrei" | "leichte_spuren" | "starke_spuren" | "beschaedigt" | "defekt") mapping: einwandfrei=Einwandfrei, leichte_spuren=Leichte Gebrauchsspuren, starke_spuren=Starke Gebrauchsspuren, beschaedigt=Beschaedigt, defekt=Defekt\n  "status": LookupValue | null, // Status (select one key: "vermietet" | "zurueckgegeben" | "ueberfaellig" | "storniert") mapping: vermietet=Vermietet, zurueckgegeben=Zurueckgegeben, ueberfaellig=Ueberfaellig, storniert=Storniert\n  "bemerkungen": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["werkzeug", "kunde"]);
        ...
        if (werkzeugName) {
          const werkzeugMatch = werkzeugbestandList.find(r => matchName(werkzeugName!, [String(r.fields.bezeichnung ?? '')]));
          if (werkzeugMatch) merged['werkzeug'] = createRecordUrl(APP_IDS.WERKZEUGBESTAND, werkzeugMatch.record_id);
        }
        ...
        return merged as Partial<Vermietung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
