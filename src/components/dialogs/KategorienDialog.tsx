...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Kategorien';
...
export function KategorienDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KategorienDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Kategorien['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'kategorien');
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'kategorie_name': (
      <div key="kategorie_name" className="space-y-1.5">
    ...
    'kategorie_beschreibung': (
      <div key="kategorie_beschreibung" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"kategorie_name": "Kategoriename", "kategorie_beschreibung": "Beschreibung"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.KATEGORIEN} recordId={recordId} />
              </div>
            )}
          </div>
