...
interface WerkzeugbestandDialogProps {
  open: boolean;
  onClose: () => void;
...
export function WerkzeugbestandDialog({ open, onClose, onSubmit, defaultValues, recordId, kategorienList, enablePhotoScan = true, enablePhotoLocation = true }: WerkzeugbestandDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Bezeichnung\n  "inventarnummer": string | null, // Inventarnummer\n  "kategorie": string | null, // Display name from Kategorien (see <available-records>)\n  "hersteller": string | null, // Hersteller\n  "modell": string | null, // Modell\n  "seriennummer": string | null, // Seriennummer\n  "anschaffungsdatum": string | null, // YYYY-MM-DD\n  "anschaffungspreis": number | null, // Anschaffungspreis (EUR)\n  "zustand": LookupValue | null, // Zustand (select one key: "neu" | "gut" | "gebraucht" | "reparaturbeduerftig" | "ausser_betrieb") mapping: neu=Neu, gut=Gut, gebraucht=Gebraucht, reparaturbeduerftig=Reparaturbeduerftig, ausser_betrieb=Ausser Betrieb\n  "standort": string | null, // Standort / Lagerort\n  "vermietbar": boolean | null, // Zur Vermietung verfuegbar\n  "tagesmietpreis": number | null, // Tagesmietpreis (EUR)\n  "kaution_betrag": number | null, // Kautionsbetrag (EUR)\n  "notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Werkzeugbestand['fields']>;
      });
