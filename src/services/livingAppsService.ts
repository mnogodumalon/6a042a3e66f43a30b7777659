...
import type { Kategorien, Werkzeugbestand, Kunden, Vermietung, CreateKategorien, CreateWerkzeugbestand, CreateKunden, CreateVermietung } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- WERKZEUGBESTAND ---
  static async getWerkzeugbestand(): Promise<Werkzeugbestand[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGBESTAND}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Werkzeugbestand[];
    return enrichLookupFields(records, 'werkzeugbestand');
  }
  static async getWerkzeugbestandEntry(id: string): Promise<Werkzeugbestand | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGBESTAND}/records/${id}`);
    const record = { record_id: data.id, ...data } as Werkzeugbestand;
    return enrichLookupFields([record], 'werkzeugbestand')[0];
  }
  static async createWerkzeugbestandEntry(fields: CreateWerkzeugbestand) {
    return callApi('POST', `/apps/${APP_IDS.WERKZEUGBESTAND}/records`, { fields: cleanFieldsForApi(fields as any, 'werkzeugbestand') });
  }
  static async updateWerkzeugbestandEntry(id: string, fields: Partial<CreateWerkzeugbestand>) {
    return callApi('PATCH', `/apps/${APP_IDS.WERKZEUGBESTAND}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'werkzeugbestand') });
  }
  static async deleteWerkzeugbestandEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WERKZEUGBESTAND}/records/${id}`);
  }
  ...
  // --- VERMIETUNG ---
  static async getVermietung(): Promise<Vermietung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERMIETUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Vermietung[];
    return enrichLookupFields(records, 'vermietung');
  }
  static async getVermietungEntry(id: string): Promise<Vermietung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.VERMIETUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Vermietung;
    return enrichLookupFields([record], 'vermietung')[0];
  }
  static async createVermietungEntry(fields: CreateVermietung) {
    return callApi('POST', `/apps/${APP_IDS.VERMIETUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'vermietung') });
  }
  static async updateVermietungEntry(id: string, fields: Partial<CreateVermietung>) {
    return callApi('PATCH', `/apps/${APP_IDS.VERMIETUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'vermietung') });
  }
  static async deleteVermietungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.VERMIETUNG}/records/${id}`);
  }
